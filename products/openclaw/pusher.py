...
def main(max_count=3, push_all=False):
    ...
    # 读取已推送版本
    pushed_counts = read_pushed_versions()
    print(f"已推送 {sum(pushed_counts.values())} 个版本")

    # 按计数过滤未推送版本（支持同版本号多条目）
    version_occurrence = Counter()
    pending_versions = []
    for v, c in all_versions:
        version_occurrence[v] += 1
        if version_occurrence[v] > pushed_counts.get(v, 0):
            pending_versions.append((v, c))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="OpenClaw 历史版本批量推送")
